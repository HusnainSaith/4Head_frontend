...
vi.mock("@/store/apiSlice", async () => {
  ...
  return { apiSlice: makeApi({ reducerPath: "api", baseQuery: (args: unknown) => { requests.push(typeof args === "string" ? { url: args } : args as Request); return { data: { data: [] } }; }, tagTypes: ["WastagePurchase", "WastageSale", "WastageStock", "WastageReport"] as const, endpoints: () => ({}) }) };
});
