...
import { useCreatePartyMutation, useListPartiesQuery } from "@/features/parties/partiesApi";
...
const columns: DataTableColumn<ShopSale>[] = [
  { id: "customer", header: "Customer", cell: (r) => r.customerParty?.name ?? "—" },
  { id: "qty", header: "Quantity (kg)", cell: (r) => `${r.quantityKg} kg`, align: "right" },
  { id: "rate", header: "Rate/kg", cell: (r) => money.format(Number(r.ratePerKg)), align: "right" },
  { id: "margin", header: "Margin/kg", cell: (r) => money.format(Number(r.profitMarginPerKg)), align: "right" },
  ...
  { id: "date", header: "Sale date", cell: (r) => String(r.saleDate).slice(0, 10) },
];
          ...
          cell: (r) => (
            <div className="flex gap-1">
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => void deleteSale(r.id).unwrap().then(() => toast.success("Sale deleted")).catch((e) => toast.error(getApiErrorMessage(e)))}
              >
                Delete
              </Button>
            </div>
...
function SaleDialog({
  ...
  const [method, setMethod] = useState<"cash" | "bank" | "credit">(sale?.paymentMethod ?? "cash");
  const [date, setDate] = useState(sale ? String(sale.saleDate).slice(0, 10) : new Date().toISOString().slice(0, 10));
  ...
  const partiesQuery = useListPartiesQuery({ type: PartyType.CUSTOMER, limit: 200 }, { skip: !open });
  const customers = partiesQuery.data?.data?.items ?? [];

  const handleClose = () => {
    setCustomerId(""); setQty(""); setRate(""); setMethod("cash");
    setDate(new Date().toISOString().slice(0, 10)); setNotes(""); setFormError("");
    setShowNewCustomer(false); setNewName(""); setNewPhone("");
    onClose();
  };
  ...
  const handleCreateCustomer = async () => {
    ...
    try {
      const result = await createParty({ partyType: PartyType.CUSTOMER, name: newName.trim(), phone: newPhone || undefined }).unwrap();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    void onSubmit({ customerPartyId: customerId || undefined, quantityKg: q, ratePerKg: r, paymentMethod: method, saleDate: date, notes: notes || undefined });
  };
    ...
    <Dialog open={open} onOpenChange={(next) => { if (!next) handleClose(); }}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {availableKg !== undefined ? `Available stock: ${availableKg} kg` : "Loading stock…"}
          </DialogDescription>
        </DialogHeader>
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <Button type="button" variant="ghost" size="sm" onClick={() => setShowNewCustomer(!showNewCustomer)}>
            ...
            {showNewCustomer ? (
              <div className="space-y-2 rounded-md border p-3">
                ...
                <Button type="button" size="sm" isLoading={partyState.isLoading} onClick={() => void handleCreateCustomer()}>
                  Create
                </Button>
              </div>
            ) : (
          ...
          <div className="space-y-1.5">
            ...
            <Input id="sale-qty" type="number" min="0.001" step="0.001" required value={qty} onChange={(e) => setQty(e.target.value)} />
            {availableKg && <p className="text-xs text-muted-foreground">Available: {availableKg} kg</p>}
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="sale-rate">Rate per kg *</Label>
            <Input id="sale-rate" type="number" min="0.01" step="0.01" required value={rate} onChange={(e) => setRate(e.target.value)} />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input id="sale-date" type="date" required value={date} onChange={(e) => setDate(e.target.value)} />
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="sale-notes">Notes</Label>
            <Input id="sale-notes" maxLength={255} value={notes} onChange={(e) => setNotes(e.target.value)} />
          </div>
