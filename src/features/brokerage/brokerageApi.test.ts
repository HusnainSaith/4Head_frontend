...
vi.mock("@/store/apiSlice", async () => {
  ...
  return { apiSlice: makeApi({ reducerPath: "api", baseQuery: (args: unknown) => { captured.push(typeof args === "string" ? { url: args } : args as Captured); return { data: [] }; }, tagTypes: ["BrokeragePurchase", "BrokerageSale", "BrokerageStock", "BrokerageReport"] as const, endpoints: () => ({}) }) };
});
