...
export const invoicesApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    getInvoices: builder.query<ApiResponse<InvoicePage>, InvoiceQueryParams | void>({
      query: (params) => ({ url: "/invoices", params: params || undefined }),
      providesTags: (result) => [
        { type: "Invoice", id: "LIST" },
        ...(result?.data?.items?.map(({ id }) => ({ type: "Invoice" as const, id })) ?? []),
      ],
    }),
    ...
    getInvoiceBySource: builder.query<ApiResponse<Invoice | null>, { sourceType: string; sourceId: string }>({
      query: ({ sourceType, sourceId }) =>
        `/invoices/by-source/${encodeURIComponent(sourceType)}/${encodeURIComponent(sourceId)}`,
      providesTags: (result) => result?.data ? [{ type: "Invoice", id: result.data.id }] : [],
    }),
